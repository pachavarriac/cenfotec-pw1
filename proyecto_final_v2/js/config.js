...
window.addEventListener('scroll', function() {
    ...
    const navVB = document.querySelector('.nav-video-background');
    const navVBA = document.querySelector('.nav-video-background::after');
    const navLV = document.querySelector('.menu-logo');
    if (window.scrollY > 0) {
        [nav, logo, navVB, navVBA, navLV].forEach(el => el.classList.add('scrolled'));
    } else {
        [nav, logo, navVB, navVBA, navLV].forEach(el => el.classList.remove('scrolled'));
    }
});
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    // if (inicioLink) {
        inicioLink.addEventListener('click', function(event) {
            ...
            const logo = document.querySelector('.menu-logo');
            const navVB = document.querySelector('.nav-video-background');
            const navLV = document.querySelector('.nav-login-video');
            
            [nav, logo, navVB, navLV].forEach(n => n.classList.remove('scrolled'));
        });
    // }
});
